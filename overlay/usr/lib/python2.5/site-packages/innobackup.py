...
MAGIC_FILE='InnoBox_Backup_Directory' #Determines whether a mountpoint 
is a backup point def _is_backup(mp):
...
def get_dates():
	return _get_dates(get_backupdir())
...
def _get_elapsed(cmd):
	...
	x = Popen(['ps','--no-headers','-C',cmd,'-o','etime']).communicate()[0]
	if x:
